import Image from 'next/image'
import Link from 'next/link'

const footerLinks = [
  { href: '/portfolio', label: 'Portfolio' },
  { href: '/about', label: 'About' },
  { href: '/services', label: 'Services' },
  { href: '/booking', label: 'Booking' },
  { href: '/contact', label: 'Contact' },
]

export default function Footer() {
  return (
    <footer className="bg-iso-dark border-t border-iso-border">
      <div className="max-w-7xl mx-auto px-6 py-16">
        <div className="grid grid-cols-1 md:grid-cols-3 gap-12 mb-12">
          {/* Brand */}
          <div className="flex flex-col items-start gap-4">
            <Link href="/" className="flex items-center gap-3">
              <Image
                src="/logos/logo-light.png"
                alt="ISO100 Photography"
                width={56}
                height={56}
                className="rounded-full"
              />
              <div>
                <p className="font-playfair text-iso-blush text-lg leading-tight">ISO100</p>
                <p className="text-xs tracking-[0.2em] uppercase text-iso-muted">Photography</p>
              </div>
            </Link>
            <p className="text-iso-muted text-sm leading-relaxed max-w-xs">
              Capturing life&apos;s most meaningful moments with artistry and intention.
            </p>
          </div>

          {/* Navigation */}
          <div>
            <p className="section-label">Navigation</p>
            <nav className="flex flex-col gap-3">
              {footerLinks.map(({ href, label }) => (
                <Link
                  key={href}
                  href={href}
                  className="text-iso-muted hover:text-iso-rose text-sm tracking-wide transition-colors duration-200"
                >
                  {label}
                </Link>
              ))}
            </nav>
          </div>

          {/* Contact */}
          <div>
            <p className="section-label">Get in Touch</p>
            <div className="flex flex-col gap-3">
              <Link
                href="/contact"
                className="text-iso-muted hover:text-iso-rose text-sm tracking-wide transition-colors duration-200"
              >
                Send a message
              </Link>
              <Link
                href="/booking"
                className="text-iso-muted hover:text-iso-rose text-sm tracking-wide transition-colors duration-200"
              >
                Book a session
              </Link>
              <div className="pt-4">
                <p className="section-label">Follow Along</p>
                <div className="flex gap-4">
                  {/* Social link placeholders – add your real URLs */}
                  <a
                    href="https://www.instagram.com/__iso100"
                    target="_blank"
                    rel="noopener noreferrer"
                    aria-label="Instagram"
                    className="text-iso-muted hover:text-iso-rose transition-colors duration-200"
                  >
                    <svg className="w-5 h-5" fill="currentColor" viewBox="0 0 24 24">
                      <path d="M12 2.163c3.204 0 3.584.012 4.85.07 3.252.148 4.771 1.691 4.919 4.919.058 1.265.069 1.645.069 4.849 0 3.205-.012 3.584-.069 4.849-.149 3.225-1.664 4.771-4.919 4.919-1.266.058-1.644.07-4.85.07-3.204 0-3.584-.012-4.849-.07-3.26-.149-4.771-1.699-4.919-4.92-.058-1.265-.07-1.644-.07-4.849 0-3.204.013-3.583.07-4.849.149-3.227 1.664-4.771 4.919-4.919 1.266-.057 1.645-.069 4.849-.069zM12 0C8.741 0 8.333.014 7.053.072 2.695.272.273 2.69.073 7.052.014 8.333 0 8.741 0 12c0 3.259.014 3.668.072 4.948.2 4.358 2.618 6.78 6.98 6.98C8.333 23.986 8.741 24 12 24c3.259 0 3.668-.014 4.948-.072 4.354-.2 6.782-2.618 6.979-6.98.059-1.28.073-1.689.073-4.948 0-3.259-.014-3.667-.072-4.947-.196-4.354-2.617-6.78-6.979-6.98C15.668.014 15.259 0 12 0zm0 5.838a6.162 6.162 0 100 12.324 6.162 6.162 0 000-12.324zM12 16a4 4 0 110-8 4 4 0 010 8zm6.406-11.845a1.44 1.44 0 100 2.881 1.44 1.44 0 000-2.881z" />
                    </svg>
                  </a>
                  <a
                    href="#"
                    aria-label="Facebook"
                    className="text-iso-muted hover:text-iso-rose transition-colors duration-200"
                  >
                    <svg className="w-5 h-5" fill="currentColor" viewBox="0 0 24 24">
                      <path d="M24 12.073c0-6.627-5.373-12-12-12s-12 5.373-12 12c0 5.99 4.388 10.954 10.125 11.854v-8.385H7.078v-3.47h3.047V9.43c0-3.007 1.792-4.669 4.533-4.669 1.312 0 2.686.235 2.686.235v2.953H15.83c-1.491 0-1.956.925-1.956 1.874v2.25h3.328l-.532 3.47h-2.796v8.385C19.612 23.027 24 18.062 24 12.073z" />
                    </svg>
                  </a>
                </div>
              </div>
            </div>
          </div>
        </div>

        <div className="border-t border-iso-border pt-8 flex flex-col sm:flex-row justify-between items-center gap-4">
          <p className="text-iso-muted text-xs tracking-wide">
            &copy; {new Date().getFullYear()} ISO100 Photography. All rights reserved.
          </p>
          <p className="text-iso-muted text-xs tracking-wide">
            Crafted with care.
          </p>
        </div>
      </div>
    </footer>
  )
}
